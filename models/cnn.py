...
class Model:
    def __init__(self, datapath, dataclass, trndata, tstdata, batchsize, **kwargs):
        ...
        model = convpool(model,16,(11,11),1,(3,3),2,2)
        model = convpool(model,32,(9,9),1,(3,3),2,2)
        model = convpool(model,64,(5,5),1,(3,3),2,2)
        model = convpool(model,128,(3,3),1,(3,3),2,2)
        model = convpool(model,256,(3,3),1,(3,3),2,2)

        model = lasagne.layers.DenseLayer(model, 512)
        model = lasagne.layers.BatchNormLayer(model)
